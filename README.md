...
A Windows desktop app for running multiple AI agents in parallel. Each agent gets its own terminal, editor, and git worktree, all in one window.
...
bun run dist      # Package as signed app
```
...
- If the laptop sleeps during a scheduled time while the app is still running, Mickey runs one catch-up execution on wake.
